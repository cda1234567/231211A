    ...
    public partial class Form1 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    tempListBox.Items.Add(mainFilePath);
                }
                ...
                if (!string.IsNullOrEmpty(mainFilePath) && File.Exists(mainFilePath))
                {
                    ...
                    // 準備第二次合併的檔案清單：主檔改用「原始主檔」，副檔改用「第一次輸出的副檔（含補料）"
                    var secondListBox = new ListBox();

                    // 主檔一律使用原始主檔
                    secondListBox.Items.Add(mainFilePath);
                    ...
                    // 發料處理完成後，重新執行扣帳以應用發料數據（主檔=原始，副檔=第一次輸出）
...
private void InitializeInventoryControls()
{
    ...
    // 移除發料管理按鈕，改為扣帳完成後自動處理
    // replenishmentButton = new Button
    // {
    //     Text = "發料處理",
    ...
    //     Font = new Font("Microsoft YaHei", 9)
    // };
    // replenishmentButton.Click += ReplenishmentButton_Click;
    // mainFileGroupBox.Controls.Add(replenishmentButton);
...
private void SetMainFileButton_Click(object? sender, EventArgs e)
{
    ...
    if (openFileDialog.ShowDialog() == DialogResult.OK)
    {
        ...
        SaveMainFileConfig();
        UpdateMainFileLabel();
        MessageBox.Show("主檔案設定成功！", "成功", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
}
...
private void PreviewButton_Click(object? sender, EventArgs e)
{
    if (string.IsNullOrEmpty(mainFilePath) || !File.Exists(mainFilePath))
    {
        MessageBox.Show("請先設定主檔案", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        return;
    }
...
private void ProcessReplenishment(string outputFolder)
{
    ...
    try
    {
        ...
        // 取得最新的輸出資料夾中的主檔案
        string mainFileToProcess = mainFilePath; // 預設使用原始主檔案
